...
export interface TextObject extends Omit<MemeText, 'id' | 'meme_id' | 'created_at' | 'order'> {
  id: number
  order: number
}
...
export default function MemeEditor({ initialImage, initialTexts, memeId, onSave }: MemeEditorProps) {
  ...
  useEffect(() => {
    ...
    if (initialTexts && initialTexts.length > 0) {
      const convertedTexts: TextObject[] = initialTexts.map((text, index) => ({
        id: index + 1,
  ...
  const addText = useCallback((x?: number, y?: number, width = 200, height = 80) => {
    const canvas = canvasRef.current
    if (!canvas) return
  ...
  const updateText = useCallback((id: number, field: keyof TextObject, value: any) => {
    setTexts((prev) =>
      prev.map((text) => {
        if (text.id === id) {
          const updated = { ...text }
          if (field === 'font_size' || field === 'width' || field === 'height' || field === 'order') {
            updated[field] = parseInt(value)
          } else {
            updated[field] = value
          }

          if ((field === 'width' || field === 'height') && updated.width && updated.height) {
            updated.font_size = calculateFontSize(updated.width, updated.height)
          }
  ...
  const selectTemplate = useCallback((templatePath: string) => {
    const img = new Image()
    img.onload = () => {
      setImage(img)
      setShowCanvas(true)
  ...
  const handleWrapperMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (target.classList.contains('resize-handle')) {
      ...
      const textBox = target.closest('.text-box-overlay') as HTMLElement
      if (textBox) {
  ...
  const handleWrapperMouseMove = useCallback((e: React.MouseEvent) => {
    if (isResizing && selectedTextId && resizeHandle) {
      ...
      const coords = getCanvasCoordinates(e)
      const left = selectedText.x - selectedText.width / 2
      const top = selectedText.y - selectedText.height / 2
      const right = selectedText.x + selectedText.width / 2
      const bottom = selectedText.y + selectedText.height / 2
      ...
      updateText(selectedTextId, 'font_size', calculateFontSize(newWidth, newHeight))
    } else if (isDragging && selectedTextId) {
      const coords = getCanvasCoordinates(e)
      const selectedText = texts.find((t) => t.id === selectedTextId)
      if (selectedText) {
        updateText(selectedTextId, 'x', coords.x - dragOffset.x)
        updateText(selectedTextId, 'y', coords.y - dragOffset.y)
      }
    }
  }, [isResizing, isDragging, selectedTextId, resizeHandle, texts, getCanvasCoordinates, dragOffset, updateText, calculateFontSize])
  ...
  const handleDocumentClick = useCallback((e: MouseEvent) => {
    if (
      ...
      !(e.target as HTMLElement).closest('.text-control-item') &&
  ...
  const handleSave = useCallback(async () => {
    const canvas = canvasRef.current
    if (!canvas || !image || texts.length === 0 || !onSave) {
      alert('Please add an image and at least one text element before saving.')
      return
    }
        ...
        <div className="sidebar-section">
          ...
          <div className="templates-grid">
            {templates.map((templatePath, index) => (
              ...
              >
                <img src={templatePath} alt={`Template ${index + 1}`} loading="lazy" />
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="canvas-section">
            <div className="canvas-container">
              ...
              >
                ...
                />
                <div ref={textBoxesOverlayRef} className="text-boxes-overlay">
                  {texts.map((text) => (
                    <TextBoxOverlay
                      key={text.id}
                      text={text}
          ...
          <div className="toolbar-content">
            ...
            <div className="toolbar-section text-controls-list">
              {texts.map((text) => (
                <div
                  key={text.id}
                  className={`text-control-item ${text.id === selectedTextId ? 'active' : ''}`}
            ...
            <div className="toolbar-section">
              ...
              <button onClick={downloadMeme} className="btn btn-download">
                Download
              </button>
            </div>
          </div>
